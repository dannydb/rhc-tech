...
},L=function(a,b){var c=b.error;b.error=function(d){c&&c(a,d,b),a.trigger("error",a,d,b)}}}.call(this),function(a,b){"object"==typeof exports&&a.require?module.exports=b(require("underscore"),require("backbone")):"function"==typeof define&&define.amd?define(["underscore","backbone"],function(c,d){return b(c||a._,d||a.Backbone)}):b(_,Backbone)}(this,function(a,b){function c(){return(0|65536*(1+Math.random())).toString(16).substring(1)}function d(){return c()+c()+"-"+c()+"-"+c()+"-"+c()+"-"+c()+c()+c()}return b.LocalStorage=window.Store=function(a){if(!this.localStorage)throw"Backbone.localStorage: Environment does not support localStorage.";this.name=a;var b=this.localStorage().getItem(this.name);this.records=b&&b.split(",")||[]},a.extend(b.LocalStorage.prototype,{save:function(){this.localStorage().setItem(this.name,this.records.join(","))},create:function(a){return a.id||(a.id=d(),a.set(a.idAttribute,a.id)),this.localStorage().setItem(this.name+"-"+a.id,JSON.stringify(a)),this.records.push(a.id.toString()),this.save(),this.find(a)},update:function(b){return this.localStorage().setItem(this.name+"-"+b.id,JSON.stringify(b)),a.include(this.records,b.id.toString())||this.records.push(b.id.toString()),this.save(),this.find(b)},find:function(a){return this.jsonData(this.localStorage().getItem(this.name+"-"+a.id))},findAll:function(){return(a.chain||a)(this.records).map(function(a){return this.jsonData(this.localStorage().getItem(this.name+"-"+a))},this).compact().value()},destroy:function(b){return b.isNew()?!1:(this.localStorage().removeItem(this.name+"-"+b.id),this.records=a.reject(this.records,function(a){return a===b.id.toString()}),this.save(),b)},localStorage:function(){return localStorage},jsonData:function(a){return a&&JSON.parse(a)},_clear:function(){var b=this.localStorage(),c=new RegExp("^"+this.name+"-");b.removeItem(this.name),(a.chain||a)(b).keys().filter(function(a){return c.test(a)}).each(function(a){b.removeItem(a)}),this.records.length=0},_storageSize:function(){return this.localStorage().length}}),b.LocalStorage.sync=window.Store.sync=b.localSync=function(a,c,d){var e,f,g=c.localStorage||c.collection.localStorage,h=b.$.Deferred&&b.$.Deferred();try{switch(a){case"read":e=void 0!=c.id?g.find(c):g.findAll();break;case"create":e=g.create(c);break;case"update":e=g.update(c);break;case"delete":e=g.destroy(c)}}catch(i){f=22===i.code&&0===g._storageSize()?"Private browsing is unsupported":i.message}return e?(d&&d.success&&("0.9.10"===b.VERSION?d.success(c,e,d):d.success(e)),h&&h.resolve(e)):(f=f?f:"Record Not Found",d&&d.error&&("0.9.10"===b.VERSION?d.error(c,f,d):d.error(f)),h&&h.reject(f)),d&&d.complete&&d.complete(e),h&&h.promise()},b.ajaxSync=b.sync,b.getSyncMethod=function(a){return a.localStorage||a.collection&&a.collection.localStorage?b.localSync:b.ajaxSync},b.sync=function(a,c,d){return b.getSyncMethod(c).apply(this,[a,c,d])},b.LocalStorage});var app=app||{};!function(){"use strict";app.Todo=Backbone.Model.extend({defaults:{title:"",completed:!1},toggle:function(){this.save({completed:!this.get("completed")})}})}();var app=app||{};!function(){"use strict";var a=Backbone.Collection.extend({model:app.Todo,setLocalStorage:function(a){this.localStorage=new Backbone.LocalStorage(a)},completed:function(){return this.filter(function(a){return a.get("completed")})},remaining:function(){return this.without.apply(this,this.completed())},nextOrder:function(){return this.length?this.last().get("order")+1:1},comparator:function(a){return a.get("order")}});app.storageKey=$(".main h1").text().toLowerCase(),app.todos=new a,app.todos.setLocalStorage(app.storageKey)}();var app=app||{};!function(a){"use strict";app.TodoView=Backbone.View.extend({tagName:"li",template:_.template(a("#item-template").html()),events:{"click .toggle":"toggleCompleted","dblclick label":"edit","click .destroy":"clear","keypress .edit":"updateOnEnter","blur .edit":"close"},initialize:function(){this.listenTo(this.model,"change",this.render),this.listenTo(this.model,"destroy",this.remove),this.listenTo(this.model,"visible",this.toggleVisible)},render:function(){return this.$el.html(this.template(this.model.toJSON())),this.$el.toggleClass("completed",this.model.get("completed")),this.toggleVisible(),this.$input=this.$(".edit"),this},toggleVisible:function(){this.$el.toggleClass("hidden",this.isHidden())},isHidden:function(){var a=this.model.get("completed");return!a&&"completed"===app.TodoFilter||a&&"active"===app.TodoFilter},toggleCompleted:function(){this.model.toggle()},edit:function(){this.$el.addClass("editing"),this.$input.focus()},close:function(){var a=this.$input.val().trim();this.$input.val(a),a?this.model.save({title:a}):this.clear(),this.$el.removeClass("editing")},updateOnEnter:function(a){a.which===ENTER_KEY&&this.close()},clear:function(){this.model.destroy()}})}(jQuery);var app=app||{};!function(a){"use strict";app.AppView=Backbone.View.extend({el:"#todoapp",statsTemplate:_.template(a("#stats-template").html()),countTemplate:_.template(a("#count-template").html()),events:{"click .toggle + label":"toggleCheckbox","click #clear-completed":"clearCompleted","click #toggle-all":"toggleAllComplete"},initialize:function(){this.allCheckbox=this.$("#toggle-all")[0],this.$input=this.$("#new-todo"),this.$header=this.$("#list-header"),this.$footer=this.$("#list-footer"),this.$main=this.$("#main"),this.$list=a("#checklist"),this.listenTo(app.todos,"add",this.addOne),this.listenTo(app.todos,"reset",this.addAll),this.listenTo(app.todos,"change:completed",this.filterOne),this.listenTo(app.todos,"filter",this.filterAll),this.listenTo(app.todos,"all",this.render),app.todos.fetch({reset:!0}),app.todos.length!==this.$list.find("li").length&&(_.invoke(app.todos.completed(),"destroy"),_.invoke(app.todos.remaining(),"destroy"),this.$list.find("li").each(function(){var b={title:a(this).text(),order:app.todos.nextOrder(),completed:!1};app.todos.create(b)}))},render:function(){var a=app.todos.completed().length,b=app.todos.remaining().length;app.todos.length?(this.$main.show(),this.$header.show(),this.$header.html(this.statsTemplate({completed:a,remaining:b})),this.$("#filters a").removeClass("active").filter('[href="#/'+(app.TodoFilter||"")+'"]').addClass("active")):(this.$main.hide(),this.$footer.hide()),this.allCheckbox.checked=!b},addOne:function(b){var c=new app.TodoView({model:b});a("#todo-list").append(c.render().el)},addAll:function(){this.$("#todo-list").html(""),app.todos.each(this.addOne,this)},filterOne:function(a){a.trigger("visible")},filterAll:function(){app.todos.each(this.filterOne,this)},newAttributes:function(){return{title:this.$input.val().trim(),order:app.todos.nextOrder(),completed:!1}},createOnEnter:function(a){a.which===ENTER_KEY&&this.$input.val().trim()&&(app.todos.create(this.newAttributes()),this.$input.val(""))},clearCompleted:function(){return _.invoke(app.todos.completed(),"destroy"),!1},toggleCheckbox:function(b){a(b.currentTarget).prev().trigger("click")},toggleAllComplete:function(){this.allCheckbox.checked,app.todos.each(function(a){a.save({completed:!1})})}})}(jQuery);var app=app||{};!function(){"use strict";var a=Backbone.Router.extend({routes:{"*filter":"setFilter"},setFilter:function(a){app.TodoFilter=a||"",app.todos.trigger("filter")}});app.TodoRouter=new a,Backbone.history.start()}();var app=app||{},ENTER_KEY=13;$(function(){"use strict";new app.AppView});